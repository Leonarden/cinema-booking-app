...
class User:
    """
    Represents an user that can buy a cinema Seat
    """
...
class Seat:
    """
    Represents a cinema seat that can be taken from an User
    """
    database = "cinema.db"
    ...
    def is_free(self):
        """
        Checks into the database if a seat is taken or not
        """
        conn = sqlite3.connect(self.database)
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def occupy(self):
        """
        Changes value of taken if in the database from 0 to 1 if seat is free
        """
        if self.is_free():
            conn = sqlite3.connect(self.database)
            conn.execute("""
...
class Ticket:
    """
    Represents a cinema Ticket purchased by an User
    """
    def __init__(self,user,price,seat_number):
        self.user = user
        self.price = price
        self.seat_number = seat_number
        self.id = "".join([random.choice(string.ascii_letters) for i in range(8)]) #set a random ascii id
